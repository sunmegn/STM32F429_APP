...
//    motor_line.set_position_max   = 4.5;
	 motor_line.set_position_min   = -5; //设置直线电机转动正向极限位置，安全起见，设置值比极值略小-1,4.5
    ...
    manipulater_throtal_D_devid = (motor_line.set_position_max - motor_line.set_position_min) / (8 / (manipulater_period*0.040)); //控制夹爪张开速度8为一个行程时间，0.40为指令执行周期，manipulater_period为降频控制机械臂
    manipulater_D2_send(MOTOR_LINE_DRIVER_ID, RS_CMD_POSITION_SET, NOACK, (uint8_t *)&motor_line.send_position, 4);//机械臂默认闭合
//	机械臂初始使夹爪处于水平方向
        ...
        if (ManipulaterQueue)
        {
            ...
            if (manipulaterQueue_Data.SonarBUTTON != 1) //声呐关闭，启动机械臂
            {
                ...
                if (manipulater_times >= manipulater_period)
                {
                    if (RotOrDir_Flag == 0)
                    {
                        motor_line.send_position -= ((float)manipulater_position.float_val[0] * manipulater_throtal_D_devid); //反向处理
