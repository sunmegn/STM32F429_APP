 ...
 * @LastEditors:Robosea
 * @LastEditTime:2020-02-25 17:49:50
...
void MessageTask_Function(void const *argument)
{
    ...
    while (1)
    {
...
#else
        do_udp();
        xQueuePeek(TempHum_Message_Queue, &temp_humi_data, 1);
        xQueuePeek(Battery_Message_Queue, &(power_data), 1);
        xQueuePeek(IMU_Message_Queue, &imu_data, 1);
        xQueuePeek(Pressure_Message_Queue, &pressure_data, 1);
        ...
        if (DataManageQueue)
        {
            ...
            /* 获得ROV控制方面反馈消息 */
            ctrldata.isRunMode   = myctrl_data.FlightMode;
            ctrldata.FB_rocker   = myctrl_data.FrontBack;
            ctrldata.LR_rocker   = myctrl_data.LeftRightMove;
            ctrldata.TURN_rocker = myctrl_data.LeftRightTurn;
            ctrldata.UD_rocker   = myctrl_data.UpDown;
            ctrldata.yaw         = imu_data.yaw;
            ctrldata.pitch       = imu_data.pitch;
            ctrldata.roll        = imu_data.roll;
            ctrldata.grayX       = imu_data.gyrox;
            ctrldata.grayY       = imu_data.gyroy;
            ctrldata.grayZ       = imu_data.gyroz;
            ctrldata.depth       = pressure_data.depth;
        }
        ...
        RovInfo_msg.sensors_data.temp = power_data.current;
        RovInfo_msg.sensors_data.humi = power_data.voltage; //FIXME 这里应该上传湿度值
        ...
        RovInfo_msg.ctrlfeedback.yawref      = speed_acc;
