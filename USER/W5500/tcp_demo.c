...
{	
	...
	switch(getSn_SR(SOCK_TCPS))											            	/*获取socket的状态*/
	{
		...
		case SOCK_ESTABLISHED:												              /*socket处于连接建立状态*/
			...
			if(len>0)
			{
				recv(SOCK_TCPS,buff1,len);								              	/*接收来自Client的数据*/
				buff1[len]=0x00; 											                  /*添加字符串结束符*/
				//printf("%s\r\n",buff);
				send(SOCK_TCPS,buff1,len);									              /*向Client发送数据*/
		  }
		  break;
   ...
   uint16 len=0;	
	...
	switch(getSn_SR(SOCK_TCPC))								  				         /*获取socket的状态*/
	{
		...
		case SOCK_ESTABLISHED: 												             /*socket处于连接建立状态*/
			...
			if(len>0)
			{
				recv(SOCK_TCPC,buff1,len); 							   		         /*接收来自Server的数据*/
				buff1[len]=0x00;  											                 /*添加字符串结束符*/
				//printf("%s\r\n",buff);
				send(SOCK_TCPC,buff1,len);								     	         /*向Server发送数据*/
			}		  
		  break;
